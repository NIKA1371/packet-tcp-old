...
# -----------------------------------------------------------
# 7. Assign IRQ affinity with load balancing across available CPU cores

# 7. Assign IRQ affinity with load balancing across available CPU cores
log "Assigning IRQ affinity for $IFACE using available CPU cores..."
CPU_COUNT=$(nproc)
...
for irq in $(grep -R "$IFACE" /proc/interrupts | awk -F: '{print $1}'); do
  echo $HEX_MASK > /proc/irq/"$irq"/smp_affinity     && log "Assigned IRQ $irq to CPU mask 0x$HEX_MASK (cores 0 to $((CPU_COUNT-1)))."     || log "Warning: Failed to assign IRQ $irq to mask 0x$HEX_MASK."
done
